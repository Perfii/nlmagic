...
## Compute the operator entanglement generation on Paulis for the PBC XYZ model for some choices of N, Jx=0.75, Jy=0.25, h=0.5 and various Jz around 0
...
while flag==False:
    ...
    paulent_list.append([1/len(pauli_list)*np.sum([Opent(Ut_list[j]@cp.array(Operator(pauli_list[i]).data)@Ut_list[j].conj().T,N1,N) for i in range(len(pauli_list))]) for j in range(len(Jz_list))])
...
data_XYZ.to_csv('XYZ_N'+str(N)+'_Jx'+str(Jx)+'_Jy'+str(Jy)+'_h'+str(h)+'.csv')
...
data_XYZ_full.to_csv('XYZ_N'+str(N)+'_Jx'+str(Jx)+'_Jy'+str(Jy)+'_h'+str(h)+'_full_time.csv')
