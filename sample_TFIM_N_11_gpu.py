...
## Compute the operator entanglement generation on Paulis for the PBC TFIM model for some choices of N, g=1 and various h around 0 (with sampling)
...
while flag==False:
    ...
    while flag_sample==False:
        ...
        paulent_sample.append([Opent(Ut_list[j]@cp.array(Operator(pauli_list[index_sample]).data)@Ut_list[j].conj().T,N1,N) for j in range(len(h_list))])
...
data_TFIM.to_csv('sample_TFIM_N'+str(N)+'_g'+str(g)+'.csv')
